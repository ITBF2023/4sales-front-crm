...
const ExpandedComponent = ({ data }) => { 
  ...
  const [documentosObservaciones, setDocumentosObservaciones] = useState([]);
  ...
  const columnsDocumentos = [
    {
      ...
      wrap: true,
      grow: 1,
      format: row => `${row.documentoNombre.slice(0, 25)}`,
    },
    {
      name: 'Tipo Documento',
    ...
    {
      name: 'Acciones',
      selector: row => mostrarBotonAccionesDocumentos(row.documentoId, row.documentoNombre),
      center: true,
      allowOverflow: true,
      grow: 2
    },
  ...
  const mostrarBotonAccionesDocumentos = (documentoObservacionId, DocumentoName) => {
    return (
    <div>
      <Button color="info" size="sm" data-tip data-for="tool-tip-preview-documento" onClick={() => getDocumentoVisualizar(documentoObservacionId)}><i className="mdi mdi-database-eye-outline"></i></Button>
      <ReactTooltip id="tool-tip-preview-documento" place="top" type="dark" effect="float">Visualizar Documento</ReactTooltip>
      <Button color="info" size="sm" data-tip data-for="tooltip-Actualizar-Documento" onClick={() => abrirModalUpdate(documentoObservacionId, DocumentoName) } ><i className="mdi mdi-account-edit"></i></Button>
      <ReactTooltip id="tooltip-Actualizar-Documento" place="top" type="dark" effect="float">Actualizar Documento</ReactTooltip>
      <Button color="info" size="sm" data-tip data-for="tool-tip-eliminar-documento" onClick={() => deleteDocumentoObservacion(documentoObservacionId)}><i className="mdi mdi-trash-can"></i></Button>
      <ReactTooltip id="tool-tip-eliminar-documento" place="top" type="dark" effect="float">Eliminar Documento</ReactTooltip>
      <Button color="info" size="sm" data-tip data-for="tool-tip-download-documento" onClick={() => getdocumentoToDownload(documentoObservacionId )}><i className="mdi mdi-cloud-download"></i></Button>
      <ReactTooltip id="tool-tip-download-documento" place="top" type="dark" effect="float">Descargar Documento</ReactTooltip>
      <Button color="info" size="sm" data-tip data-for="tool-tip-reload-documento" onClick={() => abrirModalReload(documentoObservacionId)}><i className="mdi mdi-reload"></i></Button>
  ...
  const CustomLoader = () => (
    <div align='center' style={{ padding: '24px', alignItems: 'center' }}>
      <CircularProgress />
      <div>Obteniendo documentos anexados...</div>
    </div>
  );
  ...
  // FUNCIONES DELETE DUCUMENT BITACORA
    const deleteDocumentoObservacion = async (idDocumento) => {
      if (window.confirm('Estas seguro de eliminar este documento')) {
        setFileUploadComplete(false)
        setFileUploadError(false)
        setOpenBackdrop(true);
        startUpload();
        var del = await deleteDocumentosObservacionesGestion(idDocumento)
        if(del === "OK"){
      ...
      getDocumentosObservacion(data.observaciónId)
      setdataDocumento("")
      setFilas(null)
      setdocs("")
    }
  ...
  // FUNCIONES DOWLOAD DOCUMENT BITACORA
    const getdocumentoToDownload = async (idDocumentoDowload) => {
      const getdocdowloas = await getDocumentosObservacionToDownload(idDocumentoDowload);
      //Downloaded file
      if(getdocdowloas){
        var a = document.createElement("a"); //Create <a>
        let blob = await fetch('https://4sales.com.co/SISTEMA/api/documentosGestionCaso/ObtenerDocumento/'+ idDocumentoDowload, { method: 'GET'}).then(r => r.blob());
  ...
  const UpdateDocumentoObservacion = async () => {
    let formdata = new FormData();
    formdata.append('documentoNombre', nombreArchivoUpdate)
    formdata.append('documentoId', documentoIdUpdate)
    setFileUploadComplete(false)
    setFileUploadError(false)
    setOpenBackdrop(true);
    startUpload();
    const up = await updateDocumentosObservacionesGestion(documentoIdUpdate, formdata)
    if(up === "OK"){
      getDocumentosObservacion(data.observaciónId)
      setFileUploadComplete(true)
      setTimeout(() => {
  ...
  // FUNCIONES MODAL RELOAD
    ...
    const VolverCargarArchivo = async() => {
      ...
      formdata.append('documentoId', idDocumentoReload)
      setFileUploadComplete(false)
      setFileUploadError(false)
      setOpenBackdrop(true);
      startUpload();
      const archivo = await reloadDocumentosObservacionesGestion(formdata)
      if(archivo === "OK"){
        setFileUploadComplete(true)
        setTimeout(() => {
          setOpenBackdrop(false)
      ...
      getDocumentosObservacion(data.observaciónId)
      abrirModalReload(idDocumentoReload)
    }
  //************************************************************
  ...
  const getDocumentoVisualizar = async (idDocumentoDowload) => {
    ...
    const getdocpreview = await getDocumentosObservacionToDownload(idDocumentoDowload);
    if(getdocpreview){
      let blob = await fetch('https://4sales.com.co/SISTEMA/api/documentosGestionCaso/ObtenerDocumento/'+ idDocumentoDowload, { method: 'GET'}).then(r => r.blob());
  ...
  const onRowClicked = (row, event) => {
    getDocumentoVisualizar(row.documentoId)
  }

  //******************************************************************************************/

  const getDocumentosObservacion = async(idObservacion) => {
    const observacion = await getDocumentosObservacionesGestion(idObservacion);
    if(observacion[0]){
      setDocumentosObservaciones([])
      setDocumentosObservaciones(observacion)
    }else{
      setDocumentosObservaciones([])
    }
  ...
  <div>
    ...
    <pre>
      <div style={{marginLeft:"50px", marginRight:"50px", boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px", marginTop:"10px", marginBottom:"10px"}}>
        ...
        <div style={{paddingBottom: "15px", paddingRight: "15px", paddingLeft: "15px"}}>
          ...
          <br></br>
          <div>
            <textarea style={{width: '100%',}} rows={5} disabled={true} defaultValue={data.descripcion}></textarea>
          </div>
        </div>
        { !documentosObservaciones[0] ? 
        <div style={{paddingBottom: "15px", paddingRight: "15px", paddingLeft: "15px", color: '#646d91'}}> 
          <h4>Documentos Anexados</h4>
          <div>
            <textarea style={{width: '100%',}} rows={1} disabled={true} defaultValue={"No hay documentos anexados a esta Observacion"}></textarea>
          </div>
        </div> :

        <DataTable
          columns={columnsDocumentos}
          data={documentosObservaciones}
    ...
    <Modal isOpen={modalUpdate} style={modalStyles}>
      ...
      <ModalFooter>
          <Button disabled={updateFile} onClick={UpdateDocumentoObservacion} color="primary" >Actualizar</Button>
          <Button color="secondary" onClick={() => abrirModalUpdate("","")}>Cerrar</Button>
      </ModalFooter>
    </Modal>
      ...
      onClose={abrirModalPreview}>
        ...
        <DialogContent>
          ...
          {dataDocumento == null || dataDocumento == undefined || dataDocumento == "" ? <></> : <>
            ...
            data={dataDocumento}
            width="95%" 
            height={typeDocumento.includes("image") ? "" : "600px"}
