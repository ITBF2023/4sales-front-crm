...
import '../../Usuarios/ArchivosUsuario/archivos.css'

import {Button, Modal, ModalHeader, ModalBody, ModalFooter, FormGroup, Input, Label} from 'reactstrap';
import { getMyCarpetas, getMyDocumentos, postCarpetas, postDocumentos, getUserId, deleteDocumentos, 
  updateDocumentos, updateCarpeta, deleteCarpeta, getDocumentosToDownload, reloadDocumentos} from '../../../Controller/Controller'

import Icon from '@mdi/react'
import { mdiFolderEdit  , mdiDeleteEmpty} from '@mdi/js';
import Header from "../../../components/Headers/Header.js";

const MisArchivosComponent = () => {
  ...
  // Modal Carge de Arhivos
    ...
    const [documentoDriveIdUpdate, setDocumentoDriveIdUpdate] = useState('')
  ...
  // Funciones Modal Upload
   ...
   // Funciones Modal Update
    const abrirModalUpdate = (documentoIdRecibido, documentoDriveIdRecibido, documentoTituloRecibido) =>{
      setNombreArchivoUpdate(documentoTituloRecibido)
      setDocumentoIdUpdate(documentoIdRecibido)
      setDocumentoDriveIdUpdate(documentoDriveIdRecibido)
    ...
    const UpdateDocumentoDrive = async () => {
      const update = { idDocument: documentoDriveIdUpdate, nuevoTitulo: nombreArchivoUpdate }
      setFileUploadComplete(false)
      setFileUploadError(false)
      setOpenBackdrop(true);
      startUpload();
                        ...
                        {carpetas.map((carpeta) => 
                          <div className="col-sm-6 col-md-4 col-lg-3" key={carpeta.carpetaId}>
                            ...
                            <div style={{width:"100%", float:"left"}}>
                              <i style={{float:"right"}}><Icon path={mdiDeleteEmpty} title="Delete" size={1} onClick={() => EliminarCarpetaCreada(carpeta.carpetaId)}/></i>
                              <i style={{float:"right"}}><Icon path={mdiFolderEdit } title="Edit" size={1} onClick={() => abrirModalEditCarpeta(carpeta.carpetaName,carpeta.carpetaId)}/></i> 
                            </div>
                          </div>
                        )}
                      </div>
                            ...
                            {documentos.map((documento, index)=>
                              <tr key={documento.documentoId}>
                                ...
                                <td>
                                  ...
                                  <Button color="info" size="sm" data-tip data-for="tooltip-Actualizar-Documento" onClick={() => abrirModalUpdate(documento.documentoId, documento.documentoDriveId, documento.documentoTitle) }><i className="mdi mdi-account-edit"></i></Button>
...
export default MisArchivosComponent;
