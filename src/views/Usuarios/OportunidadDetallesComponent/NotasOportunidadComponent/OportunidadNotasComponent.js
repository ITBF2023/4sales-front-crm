...
import { Button, Modal, ModalHeader, ModalBody, ModalFooter, Label, Input, Form, Container } from 'reactstrap';
...
import {getUserId, 
  ...
  getBitacorasByOportunidad, getDetallesOportunidadById, postNuevaBitacoraOportunidad, deleteBitacoraOportunidad, updateDatosBitacoraOportunidad, getNotasCompletasOportunidad} from '../../../../Controller/Controller';
...
const OportunidadNotasComponent = ({estadoCaso, final}) => {
  let path = useParams()
  ...
  const mostrarBotonAcciones = (observacionId, observacionDescrip) => {
    return <div className="AccionesBotonesBitacoras">
      ...
      <ReactTooltip id="tooltip-Actualizar-Observacion" place="top" type="dark" effect="float">Actualizar Nota</ReactTooltip>
      ...
      <ReactTooltip id="tooltip-Eliminar-Observacion" place="top" type="dark" effect="float">Eliminar Nota</ReactTooltip>
  ...
  const columns = [
    ...
    {
      name: 'Fecha de fechaCreación',
      selector: row => convert(row.fechaCreacion),
      sortable: true,
      grow: 0.5
    },
    {
      name: 'Fecha de fechaModificación',
      selector: row => convert(row.fechaModificacion),
      sortable: true,
      grow: 0.5
    },
    {
      name: 'Acciones',
      selector: row => mostrarBotonAcciones(row.bitacoraOportunidadId, row.descripcion),
      center: true,
      allowOverflow: true,
      button: true,
      width: '150px',
  ...
  // FUNCIONES MODAL UPLOAD
    ...
    const handleSubmitEdit = async () => {
      ...
      form.append('Descripción', descripcionEditObservacion);

      const observ = await updateDatosBitacoraOportunidad(idEditObservacion, form)
      if (observ.status == 201) {
        setmensajeNotificacionMostrar("Nota actualizada correctamente!")
  ...
  // Funciones Modal Crear nueva Observacion *************************************************************************************
    ...
    const handleSubmit = async () => {
      ...
      form.append('EtapaOportunidad', final);
    ...
    const EliminarObservacion = async (idObservacionEliminar) => {
      if (window.confirm("Esta Seguro de eliminar esta observación")) {
        const observ = await deleteBitacoraOportunidad(idObservacionEliminar)
        if (observ.status === 201) {
    ...
    const getObservaciones = async () => {
      if(estadoCaso === "TODOS"){
        const observaciones = await getNotasCompletasOportunidad(path.id);
        if (observaciones[0]) {
          setObservaciones([])
        ...
        const observaciones = await getBitacorasByOportunidad(path.id, estadoCaso);
        if (observaciones[0]) {
          setObservaciones([])
          setObservaciones(observaciones)
          setPending(false)
    ...
    return(
      ...
      <div style={{marginLeft:"50px", marginRight:"50px", boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px", marginTop:"10px", marginBottom:"10px"}}>
        ...
        <div style={{paddingBottom: "15px", paddingRight: "15px", paddingLeft: "15px"}}>
          ...
          <br></br>
          <div>
            <textarea style={{width: '100%',}} rows={5} disabled={true} defaultValue={data.data.descripcion}></textarea>
          </div>
        </div>
      </div>
    </pre>
    ...
    <>
        ...
        {estadoCaso === "TODOS" ? <nav >
          <ol style={{ float: "right", listStyle: "none" }}>
            <li className="nav-item d-lg-block">
              <Button color="info" onClick={openModalCrearObservacion} >+ Agregar Nota</Button>
            </li>
          </ol>
        </nav>: <></>}
      </div>
      ...
      <Modal isOpen={modalObservacion} style={modalStyles}>
        ...
        <ModalBody>
          <Form>
            ...
            <textarea className="form-control" rows="5" placeholder="Ingrese la nueva nota"
        ...
        <ModalFooter>
          <Button color="primary" disabled={enabledButton} onClick={handleSubmit}>
            Crear {creandoNota? <CircularProgress size={14} color="inherit"/>: <></>} 
          </Button>
      ...
      <Modal isOpen={modalEditObservacion} style={modalStyles}>
        ...
        <ModalBody>
          <Form>
            <Label for="descripcion">Descripcion</Label>
            <textarea className="form-control" rows="5" placeholder="Ingresa la nota"
...
export default OportunidadNotasComponent;