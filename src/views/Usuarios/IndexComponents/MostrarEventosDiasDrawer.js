...
const MostrarEventosDiaDrawer = ({evento, obtenerEventosPorMes, mesMostrar, EditarDetallesEvento}) =>{
  ...
  return (
    <div className='container-evento-fecha'>
      <ClickAwayListener onClickAway={handleTooltipClose}>
        ...
        >
           <ListItemButton style={{padding:'0 0 0 0'}} onClick={(e) => handleTooltipOpen(evento.eventoId)}>
              <Container style={{margin:'5px 0 5px 0', padding:'0 5px 0 5px', display:'flex'}}>
                <Divider orientation="vertical" variant="middle" flexItem className={evento.usuarioCreadorEvento ? 'divider-evento-detalles-lateral' : 'divider-invitado-detalles-lateral'} />
                <ListItemAvatar className='icon-centrar-top'>
                  <Container style={{display:'block', fontSize:'15px', padding:'0 10px 0 10px'}}>
                    <div>{moment(new Date(evento.fechaInicio)).format("HH:mm")}</div>
                    <div className='box-mostrar-tiempo-evento'>{ObtenerTiempoEvento(evento.fechaInicio,evento.fechaFin)}</div>
                  </Container>
                </ListItemAvatar>
                <Divider orientation="vertical" variant="middle" flexItem />
                <Container style={{display:'block',width:'70%', marginLeft:'0'}}>
                  <div className='evento-titulo-primario contenedor-evento-descripcion-mostrar'>{evento.tipoEvento === "Evento" ? <AssignmentIcon sx={{ fontSize: 18 }}/> : <VideoCameraFrontIcon sx={{ fontSize: 18 }}/>} {evento.titulo} </div>
                  <div className='evento-titulo-secundario contenedor-evento-descripcion-mostrar'>{evento.descripcion.trim().length == 0 ? "Sin descripcion" : evento.descripcion}</div>
                </Container>
...
export default MostrarEventosDiaDrawer;